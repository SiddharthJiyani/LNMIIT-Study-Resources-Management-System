    ...
    <aside className="hidden h-screen border-r bg-background md:block fixed overflow-y-auto pb-[64px] px-4">
      {navLinks.map(({ to, label, icon }) => (
        <Link
          key={to}
          to={to}
          className={`flex h-10 items-center my-[6px] rounded-md px-3 text-sm font-medium transition-colors hover:bg-muted hover:text-foreground focus:outline-none focus:ring-1 focus:ring-ring disabled:pointer-events-none disabled:opacity-50 ${
            location.pathname === to ? "bg-muted text-foreground" : ""
          }`}
        >
