...
export default function CalculateCgpa() {
    ...
    useEffect(() => {
        ...
        const fetchCourses = async () => {
            try {
                ...
                const filteredCourses = data.data.filter(course => !course.isElective);
    ...
    const calculateCgpa = () => {
        ...
        const newCgpa = (cumulativePoints + totalGradePoints) / (Number(creditsCompleted) + semesterCredits);
                ...
                <main className="flex-1 p-4 md:p-6 md:ml-[187px]">
                    <Card className="max-w-xl mx-auto shadow-lg">
                            ...
                            <div className="flex justify-between px-7">
                                <LabelInputContainer className="mt-6">
                                    ...
                                    <div className="flex">
                                        ...
                                        />
                                        <Button className="p-2 ml-1 h-[43px] w-[43px] " onClick={() => { navigate('/my-profile') }}><Pencil /></Button>
                                    </div>
                                </LabelInputContainer>
                        ...
                        <CardFooter className="flex flex-col items-center">
                            <Button onClick={calcReqSGPA} disabled={loading || !isNumeric(targetCgpa) || !isNumeric(currSemCredits) || !isNumeric(creditsCompleted)}>
                                Show required SGPA
                            </Button>
                            {reqSgpa && (
                                <>
                    ...
                    <Card className="max-w-xl mx-auto shadow-lg mt-4">
                        ...
                        <CardContent>
                            ...
                            <div className="mt-4 px-7 pt-5">
                                ...
                                {[...Array(electiveCount)].map((_, i) => (
                                    <div key={`elective-${i + 1}`} className="flex justify-between items-center mb-2">
                                        <span>Elective {i + 1} (3 credits)</span>
                                        <select
                                            value={selectedGrades[`elective-${i + 1}`] || ""}
                                            onChange={(e) => handleGradeChange(`elective-${i + 1}`, e.target.value)}
                                            className="p-2 border border-gray-300 rounded-md dark:border-gray-700"
                                        >
                                            <option value="" disabled>Select Grade</option>
                                            {Object.keys(gradePoints).map(grade => (
                        ...
                        <CardFooter className="flex flex-col items-center">
                            <Button onClick={calculateCgpa} disabled={loading || !isNumeric(creditsCompleted)}>
                                Calculate CGPA
                            </Button>
                            {cgpa && sgpa && (
                                <>
